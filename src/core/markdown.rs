...
pub fn render_markdown(text: &str) -> String {
    ...
    for line in text.lines() {
        ...
        if trimmed.starts_with('#') {
            ...
            let color = match level {1=>"\x1b[1;36m",2=>"\x1b[1;34m",3=>"\x1b[1;33m",_=>"\x1b[1;37m"};
...
fn render_inline(text: &str) -> String {
    ...
    while i < chars.len() {
        if chars[i] == '\\' && i+1 < chars.len() { out.push(chars[i]); out.push(chars[i+1]); i+=2; continue; }
        if chars[i] == '`' {
            let mut code = String::new(); i+=1;
            while i < chars.len() && chars[i] != '`' { code.push(chars[i]); i+=1; }
            if i < chars.len() { i+=1; }
            let _ = write!(out, "\x1b[36m{}\x1b[0m", code);
        } else if chars[i] == '*' && i+1 < chars.len() && chars[i+1] == '*' {
            let mut bold = String::new(); i+=2;
            while i+1 < chars.len() && !(chars[i]=='*' && chars[i+1]=='*') { bold.push(chars[i]); i+=1; }
            if i+1 < chars.len() { i+=2; }
            let _ = write!(out, "\x1b[1m{}\x1b[22m", render_inline(&bold));
        } else if chars[i] == '*' && (i+1 >= chars.len() || chars[i+1] != '*') {
            let mut italic = String::new(); i+=1;
            while i < chars.len() && chars[i] != '*' { italic.push(chars[i]); i+=1; }
            if i < chars.len() { i+=1; }
            let _ = write!(out, "\x1b[3m{}\x1b[23m", render_inline(&italic));
        } else if chars[i] == '[' {
            let mut link_text = String::new(); i+=1;
            while i < chars.len() && chars[i] != ']' { link_text.push(chars[i]); i+=1; }
            if i+1 < chars.len() && chars[i]==']' && chars[i+1]=='(' {
                let mut url = String::new(); i+=2;
